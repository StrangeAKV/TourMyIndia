...
import { MapPin, Calendar, Sparkles, Navigation, Clock, Lightbulb, Loader2, Save, BookmarkCheck, Download } from "lucide-react";
...
export function AITravelPlanner() {
  ...
  const openInMaps = (lat: number, lng: number, name: string) => {
   ...
   const exportToPDF = () => {
    ...
    doc.text(travelPlan.destination, pageWidth / 2, yPosition, { align: "center" });
    yPosition += 15;
    ...
    doc.text(`${days}-Day Travel Itinerary`, pageWidth / 2, yPosition, { align: "center" });
    yPosition += 10;
    doc.text(`Generated by TourMyIndia`, pageWidth / 2, yPosition, { align: "center" });
    yPosition += 15;
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    const overviewLines = doc.splitTextToSize(travelPlan.overview, pageWidth - 2 * margin);
    doc.text(overviewLines, margin, yPosition);
    yPosition += overviewLines.length * 5 + 10;

    // Best Time & Budget
    doc.setFontSize(10);
    doc.text(`Best Time to Visit: ${travelPlan.bestTimeToVisit}`, margin, yPosition);
    yPosition += 6;
    doc.text(`Estimated Budget: ${travelPlan.estimatedBudget}`, margin, yPosition);
    yPosition += 15;
    ...
    travelPlan.dailyItinerary?.forEach((day) => {
      ...
      day.places?.forEach((place, index) => {
        ...
        doc.setFont("helvetica", "normal");
        doc.setTextColor(80);
        const descLines = doc.splitTextToSize(place.description, pageWidth - 2 * margin - 10);
        doc.text(descLines, margin + 5, yPosition);
        yPosition += descLines.length * 4 + 3;

        doc.setTextColor(100);
        doc.text(`Duration: ${place.duration}`, margin + 5, yPosition);
        yPosition += 4;

        if (place.tips) {
          doc.setTextColor(255, 107, 53);
          const tipLines = doc.splitTextToSize(`Tip: ${place.tips}`, pageWidth - 2 * margin - 10);
    ...
    // Travel Tips
    if (travelPlan.travelTips && travelPlan.travelTips.length > 0) {
      checkPageBreak(30);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `TourMyIndia - Your Gateway to Incredible India | Page ${i} of ${pageCount}`,
        pageWidth / 2,
        290,
        { align: "center" }
      );
    ...
    // Save the PDF
    doc.save(`${travelPlan.destination.replace(/\s+/g, "-")}-Travel-Plan.pdf`);

    toast({
      title: "PDF Downloaded!",
      description: "Your travel plan has been exported as a PDF.",
    });
  };
